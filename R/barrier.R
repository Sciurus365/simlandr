...
calculate_barrier_2d <- function(l, start_location_value = 0, start_r = 0.1, end_location_value = 0.7, end_r = 0.15, base = exp(1)){
	if("2d_density_landscape" %in% class(l)) d <- l$dist
	else d <- l
	...
	p <- ggplot2::ggplot() + ggplot2::geom_point(mapping = ggplot2::aes(x = d$x[s_location_x_index], s_U)) +
	...
	result <- list(local_min_start = local_min_start,
								 ...
								 plot = p,
								 geom = ggplot2::geom_point(mapping = ggplot2::aes(x = d$x[s_location_x_index], s_U), color = "red")
	)
...
#'
#' @export
calculate_barrier_2d_batch <- function(l, start_location_value = 0, start_r = 0.1, end_location_value = 0.7, end_r = 0.15, base = exp(1)){
...
calculate_barrier_3d <- function(l, start_location_value = c(0,0), start_r = 0.1, end_location_value = c(0.7,0.6), end_r = 0.15, base = exp(1)){
	...
	result <- list(local_min_start = local_min_start,
								 ...
								 delta_U_end = saddle_point$U - local_min_end$U,
								 plot = p,
								 geom = list(ggplot2::geom_path(data = min_path, mapping = ggplot2::aes(x = x_value, y = y_value), color = "white"),
...
calculate_barrier_3d_batch <- function(l, start_location_value = c(0,0), start_r = 0.1, end_location_value = c(0.7,0.6), end_r = 0.15, base = exp(1)){
	...
	d <- d %>%
		...
		dplyr::mutate(start_x = b$local_min_start$location["x_value"],
	...
	p <- ggplot2::ggplot() + ggplot2::geom_path(data = min_path_all, mapping = ggplot2::aes(x = x_value, y = y_value)) +
		ggplot2::labs(x = l$x, y = l$y)
	...
	cols_labeller <- function(x) paste0(l$cols, ": ", x)
...
#' @describeIn calculate_barrier Plot a `barrier_landscape`
