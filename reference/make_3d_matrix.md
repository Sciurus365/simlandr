...
Currently only 3D (x, y, color) is supported. Matrices with 3D (x, y, z)
...
- rows, cols:
  ...
  The names of the parameters. `rows` can be left blank if only one
  parameter is needed.
...
A `3d_matrix_landscape` object that describes the landscape of the
